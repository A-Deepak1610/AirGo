...
- Extracts 100% genuine live DOM seat IDs and seat numbers (e.g. '9-D', '10-A') directly from EaseMyTrip.
- Clicks the real live seat element on screen.
- Advances to the final payment gateway without any dummy fallbacks.
...
def run_live_seat_extractor(visible: bool = False, pause: bool = False):
    ...
    with sync_playwright() as p:
        ...
        print("[1/5] Loading live flight search...")
        ...
        print("[4/5] Advancing to Live Seat Map...")
        checkout_page.evaluate("""() => {
        ...
        # 4. Extract 100% REAL LIVE Available Seats directly from the DOM
        print("[5/5] Reading 100% Real Live Seat Inventory from Aircraft Map...")
        real_seat_result = checkout_page.evaluate(r"""() => {
            ...
            seatLabels.forEach(el => {
                ...
                // If not occupied, add to real live selectable seats list
        ...
        if "error" in real_seat_result:
            print(f"\n[!] ERROR FROM LIVE DOM: {real_seat_result['error']}\n")
            browser.close()
            return

        print("\n" + "=" * 90)
        print("🎯 REAL LIVE SEAT EXTRACTED DIRECTLY FROM ACTIVE DOM (ZERO DUMMY DATA)")
        print("=" * 90)
        ...
        # Screenshot
        screenshot_path = "live_payment_proof.png"
        try:
            checkout_page.screenshot(path=screenshot_path, full_page=False)
            print(f"📸 Live Payment Gateway Screenshot Captured -> {screenshot_path}")
        ...
        if pause and visible:
            print("⏸️  Browser window is PAUSED on your screen for 15 seconds so you can verify...")
